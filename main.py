...
def fetch_weather_data_std(city, start_date, end_date, frequency):
    ...
    """
    api_key = 'R9MKRL4ZYUAXEUDFH34C9YZPT'
...
def plot_weather_data_std(city, start_date, end_date, frequency):
    ...
    plot_with_deviation_lines(axs[0], times, temperatures, 'Температура', 'blue', '°F')
    axs[1].set_title('Давление')
    plot_with_deviation_lines(axs[1], times, pressures, 'Давление', 'orange', 'hPa')
    axs[2].set_title('Влажность')
    plot_with_deviation_lines(axs[2], times, humidities, 'Влажность', 'green', '%')
    axs[3].set_title('Скорость ветра')
    plot_with_deviation_lines(axs[3], times, windspeeds, 'Скорость ветра', 'red', 'м/с')
...
if __name__ == '__main__':
    ...
    tk.Button(tab_current, text='Найти', command=lambda: get_weather(city_tf_current, root)).grid(row=0, column=2, padx=5,
                                                                                                  pady=5)
    # Вкладка 2: Графики с отклонениями
    ...
    # Вкладка 3: Графики со средними значениями
